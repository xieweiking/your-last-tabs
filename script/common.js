var MAX_RECORD_COUNT = 20; // TODO get this from options;
var IN_BLANK = false; // TODO get it from options;
var NEWTAB_URL = 'chrome://newtab/';
var NEWTAB = { url: NEWTAB_URL, selected : false };
var NOT_PINED_TABS = { pinned: false };

function is_newtab(tab) {
    return tab.url == NEWTAB_URL;
}
...
function save_all_windows_tabs() {
    chrome.tabs.query(NOT_PINED_TABS, function (tabs) {
        ...
        tabs.forEach(function (tab) {
            var url = tab.url;
            if (url != null && url != '' && !/^chrome\:\/\/.+/.test(url) && !url_map[url]) {
                var title = tab.title;
        ...
        if (diff == 0) {
            ...
            chrome.storage.local.get(function (items) {
                if (has_last_tabs(items)) {
                    ...
                    else if (diff > 0 && diff < legacy_tabs.length) {
